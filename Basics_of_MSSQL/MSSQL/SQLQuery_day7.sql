...
select *, ROW_NUMBER () OVER (order by department) as popularity,
RANK() OVER (order by department) as rank_num, DENSE_RANK() OVER (order by department) as Dense_rank
from Employees;


select *, ROW_NUMBER () OVER (partition by department order by EmpID) as popularity
from Employees;
